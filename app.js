...
const playDisplay = document.querySelector('.middle');
const youPickedDisplay = document.querySelector('.you-picked');
const resultDisplay = document.querySelector('.winner-loser');
const youLose = document.querySelector('.win-or-lose')
const userChoiceComponent = document.querySelector('.user-choice');
const computerChoiceComponent = document.querySelector('.computer-choice');
...
buttons.forEach(button => button.addEventListener('click', () =>{
    ...
    let classValue = button.classList[2];
    playerChoice(value, classValue);
    screenUpdateToYouPicked();
    
}))

playAgain.addEventListener('click', () => {
    playDisplay.classList.toggle('inactive');
    youPickedDisplay.classList.toggle('inactive');
    resultDisplay.classList.toggle('inactive');
    userChoiceComponent.classList.remove("rock", "paper", "scissors");
    computerChoiceComponent.classList.remove("rock", "paper", "scissors");
})

rulesButton.addEventListener('click', () => {
    /*container.classList.toggle('inactive');*/
    rulesDisplay.classList.toggle('inactive');
})
...
function playRound(user, computer){
    if(user === 'rock' && computer === 'scissors' || user === 'paper' && computer === 'rock'
      ...
      || user === 'scissors' && computer === 'scissors'){
        resultDisplay.classList.toggle('inactive');
        youLose.textContent = 'It\'s a tie';
        score.textContent = `${userScore}:${computerScore}`;
      } else {
        resultDisplay.classList.toggle('inactive');
